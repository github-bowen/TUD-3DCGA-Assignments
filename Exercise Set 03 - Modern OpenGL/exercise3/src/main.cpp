...
static glm::vec3 userInteractionSphere(const glm::vec3& selectedPos, const glm::vec3& camPos)
{
    ...
    // NOTE: selectedPos, camPos, lightPos COLLINEAR!!!!!!
    ...
    glm::vec3 direction = glm::normalize(selectedPos - camPos);  // light direction & camPos to selectedPos
    
    /* NOTE: Calculate the intersection points of the line and the sphere
    * sphere: x^2 + y^2 + z^2 = 1.5^2
    * line: P(t) =   C  + td  (C: camPos, d: direction unit vector)
    *    =>   x  = C(x) + td(x)
    *    =>   y  = C(y) + td(y)
    *    =>   z  = C(z) + td(z)
    * plug x,y,z into the sphere equation:
    *    ...... (|d|^2 = 1)
    *    t^2 + 2(Cxdx + Cydy + Czdz)t + (Cx^2 + Cy^2 + Cz^2 - 1.5^2) = 0
    * 
    *            a = 1
    *            b = 2(Cxdx + Cydy + Czdz)      = 2 dot(C, d)
    *            c = Cx^2 + Cy^2 + Cz^2 - 1.5^2 = |C|^2 - 1.5^2
    */
    float b = 2 * glm::dot(camPos, direction);
    float c = glm::dot(camPos, camPos) - radius * radius;
    float delta = b * b - 4 * c;

    if (delta < 0) {
        return glm::vec3(0.0, 0.0, 0.0);  // TODO: no solution
    }

    float t1 = (-b - sqrt(delta)) / 2, t2 = (-b + sqrt(delta)) / 2;

    if (abs(t1) > abs(t2)) {
        return camPos + t2 * direction;
    }
    return camPos + t1 * direction;
}
...
static glm::vec3 userInteractionShadow(const glm::vec3& selectedPos, const glm::vec3& selectedNormal, const glm::vec3& lightPos)
{
    ...
    glm::vec3 currentLightDirection = glm::normalize(lightPos - selectedPos);
    // cross product: new light orthogonal to both old light and normal
    glm::vec3 newLightDirection = glm::normalize(glm::cross(selectedNormal, currentLightDirection));

    float newLightDistance = 1.0f;
    //std::cout << "current light distance: " << glm::length(lightPos - selectedPos) << std::endl;
    glm::vec3 newLightPos = selectedPos + newLightDistance * newLightDirection;
    return newLightPos;
}

static glm::vec3 userInteractionSpecular(const glm::vec3& selectedPos, const glm::vec3& selectedNormal, 
    const glm::vec3& lightPos, const glm::vec3& cameraPos)
{
    ...
    glm::vec3 viewDirection = glm::normalize(cameraPos - selectedPos);  // = reflected direction

    if (glm::dot(viewDirection, selectedNormal) < 0) return lightPos;

    glm::vec3 incident = glm::normalize(glm::reflect(-viewDirection, selectedNormal));  // in fact should be minus incident
    
    glm::vec3 newLightPos = selectedPos + 1.0f * incident;
...
void imgui()
{
    ...
    std::array interactionModeNames { "Sphere", "Shadow", "Specular" };  // FIXME: fixed from wrong sequence "Shadow", "Sphere", "Specular"
    ...
    window.registerKeyCallback([&](int key, int /* scancode */, int action, int /* mods */) {
        ...
        case GLFW_KEY_R: {
            if (shiftPressed) {
                ...
                lights[selectedLightIndex].color.r = std::min(lights[selectedLightIndex].color.r + colorChange, 1.0f);
           
            } else {
                // Decrease diffuse Kd coefficient in the red channel by 0.1
                lights[selectedLightIndex].color.r = std::max(lights[selectedLightIndex].color.r - colorChange, 0.0f);
            }
            return;
        }
        case GLFW_KEY_G: {
            // Same for green.
            if (shiftPressed) {
                lights[selectedLightIndex].color.g = std::min(lights[selectedLightIndex].color.g + colorChange, 1.0f);
            } else {
                lights[selectedLightIndex].color.g = std::max(lights[selectedLightIndex].color.g - colorChange, 0.0f);
            }
            return;
        }
        case GLFW_KEY_B: {
            // Same for blue.
            if (shiftPressed) {
                lights[selectedLightIndex].color.b = std::min(lights[selectedLightIndex].color.b + colorChange, 1.0f);
            } else {
                lights[selectedLightIndex].color.b = std::max(lights[selectedLightIndex].color.b - colorChange, 0.0f);
            }
            return;
        }
        default:
